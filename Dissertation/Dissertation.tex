...
\begin{abstract}
Real-time cloth simulation, a subset of physics-based simulations, aims to generate realistic simulations of cloth-like objects using mathematics, physics, and programming. It is a complex problem to solve and many specialised methods have been developed to further the realism, while aiming to lower the computational cost. This project details the implementation of a popular cloth simulation method, extended position based dynamics, in the Unity physics framework. The Unity framework was useful in allowing features such as user interaction and experiment recording, which in turn allowed for greater testing of the methods. The implementation is compared to two other cloth simulation methods, the mass-spring system and the original position based dynamics. These three methods will be tested in various scenarios and experiments to analyse the effectiveness of these methods in simulating realistic cloth dynamics. The two key scenarios are a vertical cloth object pinned by two points, and a horizontal cloth falling on top of a rigid sphere. These were chosen as they effectively demonstrate how the cloth would move in potential projects such as games or animation. It was found that extended position based dynamics is a powerful and relatively inexpensive method to use, and improved upon position based dynamics in a few ways. One such way was decoupling the stiffness of the cloth, from the number of substeps within the simulation, a flaw that the original position based dynamics had.
...
\begin{table}[htbp]
	...
	\begin{tabular}{|c|c|c|} % Specifies three columns with centered content, and vertical lines between columns
		...
		\hline % Horizontal line at the bottom of the table
	\end{tabular}
	\caption{Frame time comparison between C\# and C++ for a mass spring system of 100x100 particles} % Table caption
	\label{tab:lang2}
\end{table}
...
\begin{table}[htbp]
	...
	\begin{tabular}{|c|c|c|} % Specifies three columns with centered content, and vertical lines between columns
		...
		\hline % Horizontal line at the bottom of the table
	\end{tabular}
	\caption{Frame time comparison between C\# and C++ for a mass spring system of 150x150 particles} % Table caption
	\label{tab:lang3}
\end{table}
...
When comparing mass-spring to the position based methods, it was clear that mass-spring performed worse on a number of aspects. The instability of using symplectic Euler integration combined with mass-spring often caused the simulation to explode at high stiffness, and thus never really was able to simulate a realistic cloth. PBD and XPBD remain unconditionally stable, and this was proven by utilising both the wind force and the user interaction to test the constraints when extreme forces were applied. The stiffness was easily controllable too, through the use of Unity public ranges, they were modifiable in real time. XPBD sucessfully improved upon PBD by fixing the stiffness issue and the overall simulation performed better through the use of substeps. \\

The originals aims of the initial project overview were met, which was to implement XPBD within a framework and analyse its effectiveness through testing. Extra features such as collision, user interaction and recording were also implemented to help the evaluation. The project would've benefited from an additional scenario showcasing how the method performs on clothing, but this wasn't implemented due to time constraints.
\\

Overall, this project was a success, as it showcased the way this cloth simulation method was implemented in a framework, and allowed for various experiments to be conducted to analyse the efficiency of XPBD in simulating cloth dynamics.
...
\begin{minipage}{\textwidth}
	\includepdf[offset=0 -2cm,scale=0.65,pages=1,pagecommand={\section{Project Overview}\label{ipo}}]{appendices/IPO.pdf}
\end{minipage}
\includepdf[scale=0.65,pages=2,pagecommand={}]{appendices/IPO.pdf}

\begin{minipage}{\textwidth}
	\includepdf[offset=0 -2cm,scale=0.65,pages=1,pagecommand={\section{Second Formal Review Output}\label{interim}}]{appendices/interim.pdf}
\end{minipage}
\includepdf[scale=0.65,pages=2,pagecommand={}]{appendices/interim.pdf}

\begin{minipage}{\textwidth}
	\includepdf[offset=0 -2cm,scale=0.65,pages=1,pagecommand={\section{Diary Sheets}\label{meeting}}]{appendices/meeting.pdf}
\end{minipage}
\includepdf[scale=0.65,pages=2,pagecommand={}]{appendices/meeting.pdf}
\includepdf[scale=0.65,pages=3,pagecommand={}]{appendices/meeting.pdf}
\includepdf[scale=0.65,pages=4,pagecommand={}]{appendices/meeting.pdf}
\includepdf[scale=0.65,pages=5,pagecommand={}]{appendices/meeting.pdf}
\includepdf[scale=0.65,pages=6,pagecommand={}]{appendices/meeting.pdf}
\includepdf[scale=0.65,pages=7,pagecommand={}]{appendices/meeting.pdf}

\begin{minipage}{\textwidth}
	\includepdf[offset=0 -1cm,scale=0.7,pages=1,pagecommand={\section{Poster}\label{poster}}]{appendices/poster.pdf}
\end{minipage}
\end{appendices}
