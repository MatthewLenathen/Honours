...
todo - section about other ways to simulate cloth with particles, not just with springs. PBD fits into this category but since it is the focus of the project, it has its own section later.
\subsubsection{Continuum}
todo - section will talk about the continuum approach, doesn't deal with particles or springs but on the actual triangles themselves. Finite element methods falls under this section
\subsubsection{Alternative Methods for Cloth Simulation}
todo - this section will talk about other more niche ways for cloth simulation such as simulCap and neural networks
...
In the paper, Müller et al also discuss their implementation of a cloth simulator for use in games. Their simulator accepts a triangle mesh and creates two constraints, stretching and bending constraints. Much like the springs in mass-spring systems, these constraints force the positions of the particles to satisfy them in order to continue the simulation, this relates to the loop in line 15 of the algorithm.

... todo: finish this section by comparing the results and analysing them.

\paragraph{Extended Position Based Dynamics}
todo: talk about the extended position based dynamics paper

\paragraph{Other PBD methods}
todo: can talk about other implementations that utilize PBD, such as projective dynamics by Bouaziz et al. bridging finite element methods and PBD
